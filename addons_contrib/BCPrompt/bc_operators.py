...
class ConsoleDoAction(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        DONE = {'FINISHED'}
        if in_scene_commands(context, m):
            return DONE

        elif in_search_commands(context, m):
            return DONE

        elif in_sverchok_commands(context, m):
            return DONE

        elif in_core_dev_commands(context, m):
            return DONE

        elif in_modeling_tools(context, m):
            return DONE

        elif in_upgrade_commands(context, m):
            return DONE
